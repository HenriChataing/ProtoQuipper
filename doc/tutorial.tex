...
\end{verbatim}
The output format is determined by the option provided to \verb#-f#. In the above case, \verb#v# selected the visual format. The default format option is \verb#ir#, which outputs the intermediate representation of quantum circuits specified in the context of the QCS program. 
...
Note that no term of the form $(t,C,a)$ appears in table 
\hyperref[terms]{\ref*{terms}}. This is because the $(t,C,a)$ construct is 
...
In the code of the module \verb#Bell# above, the first two occurrences of 
the \verb#let# binder are not instances of the formal binder defined 
in subsection \hyperref[ssec-types-and-terms]{\ref*{ssec-types-and-terms}} 
as they do not bind a pair. Rather, the binding introduces let-polymorphism (an integral part of all the ML-based languages). In fact, we authorize binding 
on arbitrary \emph{patterns} which are defined by the following grammar:
\begin{center}
\begin{tabular}{rcl}
...
\end{tabular}
\end{center}
As is common in functional programming languages, we introduce shorthand notations to simplify the code. Figure \ref{sugar} gives the list of the notations introduced in Proto-Quipper. 
...
Proto-Quipper allows the use unparenthesized tuples of arbitrary size. 
...
\begin{itemize}
  \item \verb#:ctx# which lists the variables declared in the current context, 
...
\begin{itemize}
  ...
  \item The type is followed by a list of type arguments (the list can 
  be empty) 
...
\end{verbatim}
In Proto-Quipper, only functions may be recursively defined. In this respect, 
Proto-Quipper differs from Haskell since the latter allows recursive values 
like lists of infinite size.
...
The \verb#List# module contains, among others, the \verb#sf_last# function, whose type is:
\begin{verbatim}
val sf_last <: !(list a -> (a, list a))
\end{verbatim}
The function imputs a list and returns returns the last element of a list. Due to linearity, it also returns the rest of the list. The module \verb#Core#
provides basic integer operations including \verb#+ ^ - * ..#. 

...
This will provide a basis for extension to the full language in 
the future. The current version of Proto-Quipper (0.1) therefore 
focuses on the following key feature of Quipper: the ability to 
generate, and act on, quantum circuits. Moreover, Proto-Quipper 
is equipped with a linear type system that guarantees that 
programs do not violate the no-cloning property.  

Proto-Quipper is based on the Quantum Lambda Calculus (QLC, see 
\cite{SeVa}). In the QLC, the reduction relation is defined on 
\emph{closures}, where a closure is essentially a pair $[C,t]$ 
consisting of a term $t$ and a quantum state $C$. The state is 
a unit vector in a complex Hilbert space and $t$ is a term whose 
free variables are linked to the qubits composing $C$. The quantum 
...
The reduction in Proto-Quipper will similarly be defined on 
closures. However, a closure $[C,t]$ will now consist of a term 
$t$ and a \emph{circuit state} $C$. This state represents the 
circuit being currently built. Instead of having a quantum device 
capable of performing quantum operations, we will therefore assume 
that we have a \emph{circuit constructor} capable or performing 
certain circuit building operations (appending gates,\ldots). 
The reduction will then be defined as a non-probabilistic rewrite 
procedure on closures. As in the QLC, some redexes will affect the 
state by sending instructions to the circuit constructor. For 
example: ``Append a Hadamard gate to wire number 3''.

In what follows, we distinguish between the \emph{core} 
of Proto-Quipper, which represents the proved type-safe fragment 
of the language, and the \emph{extensions} of the language, 
containing newly implemented features of Proto-Quipper that 
are currently under development. 


\section{Core Proto-Quipper}

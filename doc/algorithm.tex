...
For the purpose of type inference, it would seem natural to define the type system of $\QP'$ by extending the types of $\QP$ with type variables. This would give the following grammar for types:
	...
	\end{tabular}
	\end{center}
This would have been sufficient if the types were not linear. However, we need to be able to separately discuss the \textit{form} of a type (for example $qbit$, $bool \otimes bool$) and its \textit{linearity} (whether the type is duplicable or not). We therefore introduce a new grammar which, though less intuitive, offers more control over the form of a type.
...
\begin{itemize}
	...
	\item $\texttt{Flag}$ consists of \textit{flag variables} and the constants $0$ and $1$. They parametrize the linearity of a type.
		...
		of any quantum bit will be $!^0 qbit$.
\end{itemize}
...
Before we can define the type unification algorithm (Algorith~\ref{alg-type-uni} below), we need an ordering of type variables that ensures termination and gives an upper

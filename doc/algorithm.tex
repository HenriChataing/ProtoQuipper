\title{ Rapport de stage de recherche }
...
\def\type{\texttt{Type}}
\def\qdatatype{\texttt{QDataType}}
\def\qdata{\texttt{QData}}
\def\spec{\texttt{Spec}}

\def\binding{\mathfrak{b}}
...
terms and type system of the language of reference of the formalization, which we name QP', by opposition to QP or Proto-Quipper. 
...
\begin{defn} The natural way to implement the type system of QP' would be to select in the set of types of QP those associated with the
	terms of QP', and extend them with type variables. This would give :
		...
		\end{tabular}
		\end{center}
	This would have been sufficient had the types not been linear. Because we need to be able to separately discuss the \textit{value}
	and the \textit{linearity} of any type, a new grammar is introduced which, though less intuitive, offers more control over the form
	of a type :
		\begin{center}
		\begin{tabular}{rl}
			$\texttt{Flag}~ ::=$ & $n ~|~ 0 ~|~ 1$ \\		
	...
	Flags, linear types and types are to be interpreted as follow :
		\begin{itemize}
			\item $\texttt{LinearType}$ is the subset of types that are always linear. They will always correspond to terms that can not be duplicated,
				such as qbits.
			\item Depending of the value of their prefixing flag, $\texttt{Type}$ can be duplicable or not. This is intended as the general type of terms.
...
\begin{remark} The typing rules of QP' are mostly the same as those of QP, but for the different syntax of types and the special
	case of the tensors typing rules. Recall that the typing rule for $\pair{a}{b}$ was in QP (without the quantum contexts)\,:
		\begin{prooftree}
		...
		\end{prooftree}
	What it says is that the pair $\pair{a}{b}$ is duplicable if and only if both $a$ and $b$ are. However, the grammar of QP' types doesn't
	allow us to move the $!$ around. This is why the rule $(\otimes.I)$ of QP' uses the type $!^p(!^nA \otimes \,!^mB)$ with the condition
	that $p$ can be $1$ if and only if both $n$ and $m$ are equal to one. This results in the duplicable pair $\pair{a}{b}$ having the
...
\begin{thm}{\bf Soundness and completeness} Let $t$ be a QP' term. There exists two transformations $\tau$ and
	$\bar \tau$ applying on types, respectively from QP to QP' types, and QP' to QP types, verifying the properties :
	\begin{enumerate}
...
\begin{defn} We introduce \textit{substitutions}, defined as a finite list of mappings $(\alpha \mapsto A)$ or $(n \mapsto 0 ~|~ 1)$
	...
	$$
 	...
 	The definition of $\sigma$ ensures that the kind of types is left untouched : linear types are mapped to linear types, and so are
 	types. The substitution $\sigma$ is extended to contexts by defining 
 		$$\sigma (x_1 : T_1 \dots x_n : T_n) = (x_1 : \sigma T_1 \dots x_n : \sigma T_n)$$
	If $\sigma$ and $\tau$ are substitutions, we write $\sigma \circ \tau$ for the composition of $\sigma$ by $\tau$, defined as follow
 	...
 	\textit{idempotent} if it verifies $\sigma \circ \sigma = \sigma$. It can be proved in particular that $\sigma$ is idempotent if and
...
type $T$ in the context $\Gamma$, then for all substitutions instances $\sigma \Gamma \vdash t : \sigma T$.
...
variables of $\Gamma, T$ leads to the problem of type inference. The formalize this, we need a concrete way to substitute variables
to obtain a valid typing judgement.
...
\begin{exmp} Let $f$ be $\lambda x.\pair{x}{x}$ of type $!^n \alpha$, $a$ of type $!^m \beta$ and $t = f \, a$. Then
	\begin{center}
	\begin{tabular}{l}
		$[\alpha ~\mapsto~ !^1(!^1A \multimap !^0B),~~ \beta ~\mapsto~ !^1A,~~ \gamma ~\mapsto~ !^0B]$ \\
		$[\alpha ~\mapsto~ !^0(!^1A \multimap !^1B),~~ \beta ~\mapsto~ !^1A,~~ \gamma ~\mapsto~ !^1B]$ \\
		$[\alpha ~\mapsto~ !^0(!^1int \multimap !^0(!^1int \otimes !^1int)),~~ \beta ~\mapsto~ !^1int,~~ \gamma ~\mapsto~ !^0(!^1int \otimes !^1int)]$
	\end{tabular}
	\end{center}
...
\begin{defn} We introduce sub-typing constraints $T \prec: U$ and $A \prec: B$ defined over types and linear types.
  ...
  for $m, n\in \fset$.
  Though all flag constraints are written in the same form $. \le .$, they can hold different meanings, for example $n \le 0$ can
  be viewed as $n = 0$, $1 \le m$ is the constraint $m = 1$, and $n \le m$ can be interpreted as $n = 1 \Rightarrow m = 1$.
...
\begin{exmp} The same example term $\lambda x.\pair{x}{x}$ can be typed using the constraint typing rules, with the derivation
	\begin{prooftree}
		...
		\end{prooftree}
	the annotation sets $\chi$ are omitted to avoid surcharging the tree. Thus, we have
...
collecting all the constraints that must be met for it to exist in $\lset$. $\lset$ characterizes the possible valid instantiations of
...
We now have two different ways of characterizing the possible instantiations of $\Gamma \vdash t : T$ as
 	\begin{enumerate}
 		...
 		\item Supposing the relation $\Gamma \vdash_\lset t : T$, a the solutions for $(\Gamma, t, T, \lset)$ in the sense of the definition
 			\ref{alg_problem}.
 	\end{enumerate}

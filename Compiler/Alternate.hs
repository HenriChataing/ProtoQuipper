...
module Compiler.Alternate where
...
data CExpr =
  ...
  | CApp Value [Value] Variable CExpr              -- ^ Function application: @t u@. The result is recorded in a variable.
  | CTailApp Value [Value]                         -- ^ Tail call. The call returns directly.
...
data CUnit = CUnit {
  ...
  extern :: [FunctionDef],                 -- ^ Functions accessible outside of the compile unit.
  local :: [FunctionDef],                  -- ^ Functions defined locally.
...
convert_to_alt :: (IQLib, IBuiltins)              -- ^ Interfaces to the QLib and Builtins modules.
...
convert_to_alt dict vals c (S.EVar x) =
...
convert_to_alt dict vals c (S.EGlobal x) =
  convert_to_alt dict vals c (S.EVar x)

convert_to_alt dict vals c (S.EInt n) =
  c (VInt n)

convert_to_alt dict vals c (S.EBool b) =
  c (if b then VInt 1 else VInt 0)

convert_to_alt dict vals c S.EUnit =
  c (VInt 0)

convert_to_alt dict vals c (S.ETuple []) =
  c (VInt 0)

convert_to_alt dict vals c (S.ETuple elist) = do
  x <- create_var "x"
  aux elist (\w -> do
        cx <- c (VVar x)
  ...
  where
    aux l c = do
      let aux' [] w = c (List.reverse w)
          aux' (e:es) w = convert_to_alt dict vals (\v -> aux' es (v:w)) e
      aux' l []

convert_to_alt dict vals c (S.EAccess n x) = do
  y <- create_var "x"
  cy <- c (VVar y)
...
convert_to_alt dict vals c (S.EApp (S.EVar f) arg) = do
  x <- create_var "x"    -- result of the application
  case value vals f of
    VLabel f -> do
        cx <- c (VVar x)
        convert_to_alt dict vals (\arg ->
              return $ CApp (VLabel f) [arg] x cx) arg
    _ -> do
        cx <- c (VVar x)
        convert_to_alt dict vals (\f ->
              convert_to_alt dict vals (\arg ->
                    return $ CApp f [arg] x cx) arg) (S.EVar f)

convert_to_alt dict vals c (S.EApp (S.EGlobal f) arg) =
  convert_to_alt dict vals c (S.EApp (S.EVar f) arg)

convert_to_alt dict vals c (S.EApp f arg) = do
  x <- create_var "x"       -- result of the application
  cx <- c (VVar x)
  convert_to_alt dict vals (\f ->
        convert_to_alt dict vals (\arg ->
...
convert_to_alt dict vals c (S.ESeq e f) = do
  convert_to_alt dict vals (\z -> convert_to_alt dict vals c f) e

convert_to_alt dict vals c (S.ELet x e f) = do
  convert_to_alt dict vals (\z -> do
        convert_to_alt dict (IMap.insert x z vals) c f) e

convert_to_alt dict vals c (S.EIf e f g) = do
  k <- create_var "k"
  x <- create_var "x"
  cx <- c (VVar x)
  f' <- convert_to_alt dict vals (\z -> return $ CTailApp (VVar k) [z]) f
  g' <- convert_to_alt dict vals (\z -> return $ CTailApp (VVar k) [z]) g
  convert_to_alt dict vals (\e ->
        return $ CFun k [x] cx $
                 CSwitch e [g', f']) e

convert_to_alt dict vals c (S.EMatch e blist) = do
  ...
  elist' <- List.foldl (\rec (_, e) -> do
        es <- rec
        e' <- convert_to_alt dict vals (\z -> return $ CTailApp (VVar k) [z]) e
        return $ e':es) (return []) slist
  convert_to_alt dict vals (\e ->
        return $ CFun k [x] cx $
                 CSwitch e (List.reverse elist')) e

convert_to_alt (iqlib, ibuiltins) vals c (S.EBuiltin s) =
  case lookup s iqlib of
    Just v -> c (VGlobal v)
    Nothing ->
        case lookup s ibuiltins of
          Just v -> c (VGlobal v)
          Nothing -> fail $ "CPS:convert_to_alt: undefined builtin operation: " ++ s
  ...
  (cu, _) <- List.foldl (\rec (S.DLet f e) -> do
        ...
        case e of
          S.EFun x c -> do
              ...
              body <- convert_to_alt dict vals (\z -> return $ CRet z) c
          ...
          _ -> do
              ...
              init <- convert_to_alt dict vals (\z -> return $ CSet f z) e
    ...
    ) (return (CUnit { extern = [], local = [], vglobals = [], imports = imported}, ivals)) decls
  return cu { local = List.reverse $ local cu, extern = List.reverse $ extern cu, vglobals = List.reverse $ vglobals cu }
...
closure_conversion_aux (CTailApp (VVar f) args) = do
  f' <- create_var "f"
  return ( CAccess 0 (VVar f) f' $
           CTailApp (VVar f') (VVar f:args),
           f:(List.concat $ List.map free_var args))

closure_conversion_aux (CTailApp (VLabel f) args) = do
  return ( CTailApp (VLabel f) (VInt 0:args),
           f:(List.concat $ List.map free_var args))
...
instance PPrint CUnit where
  genprint _ [fvar] cu =
    ...
    let (gdef, ginit) = List.unzip $ List.map (\(g, e) ->
          (text (fvar g), print_alt Inf fvar e)) (vglobals cu) in

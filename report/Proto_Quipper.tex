...
\]
\[
  \infer[(\rul{unit})]{[C,\letin{*}{*}{a}]\to[C,a]}{}
\]
\[
...
The circuit looks as follows (when reading this report in a PDF
viewer, more details can be viewed by switching to an appropriate zoom
level):
\[ \resizebox{\textwidth}{!}{\includegraphics{bwt}}
\]
Here is the beginning of the circuit:
\[ \resizebox{\textwidth}{!}{\includegraphics[clip=true,trim=0in 0in 27in 0in]{bwt}}
\]
We note that, since Proto-Quipper does not have any facilities for
processing command-line options in user-level programs, to change the
parameter (such as $n$, $s$, or the welding vectors), the user must
update the source code of the \verb!main! function in \verb!bwt/bwt.qp!.

\subsection{Correctness}

The correctness of the Binary Welded Tree algorithm implementation can
be established in a modular fashion. First, the correctness of the
diffusion step can be verified by direct inspection of the generated
circuits, since the diffusion step is small and very regular. Second,
the correctness of the oracle can be verified by simulation, keeping
in mind that the oracle only uses classical gates, which can therefore
be simulated efficiently. Here, the use of the PLATO intermediate
representation is convenient, as the circuit can be simulated by
simply copying and pasting the relevant circuit fragments and feeding
them into the Quipper stand-alone simulation tool. For example, to
simulate the first oracle (for the color $c=0$), one should
copy-and-paste the block of gates from lines 7 to 193 of the IR,
starting with ``\verb!QInit0(5)!'' and ending with
``\verb!QTerm0(16)!''. Assuming that the circuit fragment is pasted
into a file \verb!oracle.ir!, the circuit can then be simulated using
the tool \verb!Simulate!, which can be found in the directory
\verb!Programs/Tools! of the Quipper distribution:
\begin{code}
\begin{verbatim}
Simulate < oracle.ir
\end{verbatim}
\end{code}
The resulting simulation shows the output corresponding to every
possible input state, and starts like this:
\begin{code}
\begin{verbatim}
|00000> ->
           |00000000000> 1.000000000000000000000000000000
|00001> ->
           |00001000100> 1.000000000000000000000000000000
|00010> ->
           |00010000010> 1.000000000000000000000000000000
|00011> ->
           |00011000001> 1.000000000000000000000000000000
\end{verbatim}
\end{code}
The correctness of the outputs can be verified either manually, by
direct comparison with its specification, or by comparing it to the
output of other implementations of the Binary Welded Tree algorithm.
